...
def _draw(app: Page, target, x0, y0, x1, y1):
    drag(app, target, x0, y0, x1, y1)
    wait_for_app_run(app)


def test_two_canvases_do_not_interfere(app: Page):
    canvas_a = canvas(app, 0)
    _draw(app, canvas_a, 20, 20, 80, 80)

    assert len(read_json(app, 0)["objects"]) == 1
    assert read_json(app, 1)["objects"] == []


def test_undo_history_survives_an_unrelated_rerun(app: Page):
    canvas_a = canvas(app, 0)
    _draw(app, canvas_a, 20, 20, 80, 80)
    _draw(app, canvas_a, 100, 100, 150, 150)
    assert len(read_json(app, 0)["objects"]) == 2
...
def test_session_state_survives_canvas_creation(app: Page):
    ...
    assert read_json(app, 2)["foo"] == "value"
